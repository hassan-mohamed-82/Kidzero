import { mysqlTable, varchar, text, date, boolean, int, mysqlEnum, timestamp, unique, } from "drizzle-orm/mysql-core";
// ENUMS
export const userStatusEnum = ["pending", "approved", "rejected"];
export const userRoles = ["member", "guest"];
export const popUpsStatus = ["active", "disabled"];
// ADMINS
...
export const userCompetition = mysqlTable("user_competition", {
    id: varchar("id", { length: 36 }).primaryKey(),
    userId: varchar("user_id", { length: 36 })
        .notNull()
        .references(() => users.id),
