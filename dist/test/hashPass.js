...
async function testBcrypt() {
    ...
    const correctMatch = await bcrypt_1.default.compare(password, hashedPassword);
    console.log("Correct password match:", correctMatch); // Should be true
    // Test with wrong password
    const wrongPassword = "wrongPassword";
    const wrongMatch = await bcrypt_1.default.compare(wrongPassword, hashedPassword);
    console.log("Wrong password match:", wrongMatch); // Should be false
    console.log("\n--- Hashing the same password twice ---\n");
    // Hash the same password again
    const hashedPassword2 = await bcrypt_1.default.hash(password, saltRounds);
    ...
    console.log("Do they both match the original?", await bcrypt_1.default.compare(password, hashedPassword2)); // Should be true
}
