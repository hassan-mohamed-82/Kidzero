...
import { competitions, competitionsImages, userCompetition, } from "../../models/schema";
...
export const getCompetition = async (req, res) => {
    ...
    if (!competition)
        throw new NotFound("Competition not found");
...
export const createCompetition = async (req, res) => {
    ...
    await db.transaction(async (tx) => {
        await tx.insert(competitions).values({
            ...
            name,
            description,
            mainImagepath: await saveBase64Image(mainImagepath, id, req, "competitionsMain"),
        ...
        if (images !== undefined && Object.keys(images).length > 0) {
            images.forEach(async (imagePath) => {
                ...
                await tx.insert(competitionsImages).values({
                    id: imageId,
                    competitionId: id,
                    imagePath: await saveBase64Image(imagePath, imageId, req, "competitionsImages"),
                });
            });
        }
    });
...
export const getCompetitionUsers = async (req, res) => {
    const id = req.params.id;
...
export const getCompetitionImages = async (req, res) => {
    const id = req.params.id;
    const [competitionExists] = await db
        .select()
        .from(competitions)
...
export const deleteCompetition = async (req, res) => {
    ...
    if (!competitionExists)
        throw new NotFound("Competition not found");
    const deleted = await deletePhotoFromServer(new URL(competitionExists.mainImagepath).pathname);
    if (!deleted)
    ...
    await db.transaction(async (tx) => {
        ...
        if (images && images.length > 0) {
            images.forEach(async (img) => {
                const deleted = await deletePhotoFromServer(new URL(img.imagePath).pathname);
                if (!deleted)
    ...
    await db.transaction(async (tx) => {
        ...
        if (Array.isArray(images)) {
            ...
            for (const img of additions) {
                const imageId = uuid4v();
                const savedPath = await saveBase64Image(img.imagePath, imageId, req, "competitionsImages");
                await tx.insert(competitionsImages).values({
                    id: imageId,
                    competitionId: id,
                    imagePath: savedPath,
...
// Not Complete
export const updateCompetitionImages = async (req, res) => { };
export const removeCompetitionUser = async (req, res) => {
    const id = req.params.id;
    ...
    const [userInComp] = await db
        .select()
        .from(userCompetition)
        .where(and(eq(userCompetition.competitionId, id), eq(userCompetition.userId, userId)));
    if (!userInComp)
        throw new NotFound("User not registered in this competition");
    await db
        .delete(userCompetition)
        .where(and(eq(userCompetition.competitionId, id), eq(userCompetition.userId, userId)));
    SuccessResponse(res, { message: "User removed from competition successfully" }, 200);
};
