...
export const updateUser = async (req, res) => {
    ...
    if (newUser.imageBase64) {
        ...
        newUser.imagePath = await saveBase64Image(newUser.imageBase64, id, req, "users");
    }
...
export const deleteUser = async (req, res) => {
    ...
    if (!user)
        throw new NotFound("User not found");
    if (user.imagePath) {
        const deleted = await deletePhotoFromServer(new URL(user.imagePath).pathname);
        if (!deleted)
...
export const approveUser = async (req, res) => {
    ...
    await sendEmail(user.email, "Your account has been approved", "Congratulations! Your account has been approved by the admin. You can now log in and start using our services.");
...
export const rejectUser = async (req, res) => {
    ...
    const result = await db
        ...
        .where(eq(users.id, id));
    await sendEmail(user.email, "Your account has been Rejected", "Unfortunately, your account was rejected. The Reason is " +
