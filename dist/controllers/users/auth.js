...
import { ForbiddenError, NotFound, UnauthorizedError, UniqueConstrainError, } from "../../Errors";
    ...
    if (existing) {
        ...
        if (isVerified) {
            if (existing.email === email)
                throw new UniqueConstrainError("Email", "البريد الإلكتروني مستخدم بالفعل");
            if (data.phoneNumber && existing.phoneNumber === data.phoneNumber)
                throw new UniqueConstrainError("Phone Number", "رقم الجوال مستخدم بالفعل");
        }
        ...
        await sendEmail(existing.email.trim().toLowerCase(), "Email Verification", `Your verification code is ${code}`);
...
export const login = async (req, res) => {
    ...
    if (user.status !== "approved") {
        throw new ForbiddenError("الحساب غير موافق على التسجيل. يرجى الانتظار حتى يتم الموافقة عليه");
    }
    if (!user.isVerified) {
...
export const resetPassword = async (req, res) => {
    ...
    const [rowcode] = await db
        .select()
        .from(emailVerifications)
        .where(and(eq(emailVerifications.userId, user.id), eq(emailVerifications.code, code)));
    if (!rowcode)
...
export const resendVerificationCode = async (req, res) => {
    ...
    await sendEmail(user.email, "Email Verification", `Your new verification code is ${code}`);
