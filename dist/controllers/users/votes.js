...
import { userVotes, userVotesItems, votes, votesItems, } from "../../models/schema";
...
export const getAllVotes = async (req, res) => {
    const votesList = await db
        .select()
        .from(votes)
        .where(and(gte(votes.endDate, new Date()), lte(votes.startDate, new Date())));
...
export const getVote = async (req, res) => {
    ...
    if (prevVote)
        throw new ConflictError("you voted before");
    const votesItemsList = await db
        .select()
        .from(votesItems)
        .where(eq(votesItems.voteId, voteId));
    if (!votesItemsList)
        throw new BadRequest("Something Wrong...");
    SuccessResponse(res, { votename: vote.name, selectedList: votesItemsList }, 200);
};
export const submitVote = async (req, res) => {
    ...
    const [existingVote] = await db
        ...
        .where(and(eq(userVotes.userId, userId), eq(userVotes.voteId, voteId)));
    if (existingVote)
        throw new ConflictError("You already voted for this vote");
    ...
    if (invalid)
        throw new BadRequest("One or more selected items are invalid");
...
export const voteResult = async (req, res) => {
    ...
    const userId = req.user.id;
    // نتائج التصويت
    const [results] = await pool.query("CALL GetVoteResults2(?)", [voteId]);
    const finalResult = results[0];
    if (!finalResult || finalResult.length === 0) {
        throw new NotFound("No vote results found");
    }
    // 1) هات user_vote.id للمستخدم
    const [userVote] = await pool.query("SELECT id FROM user_votes WHERE vote_id = ? AND user_id = ? LIMIT 1", [voteId, userId]);
    let votedItemIds = [];
    if (userVote.length) {
        const userVoteId = userVote[0].id;
        // 2) جلب كل الـ item_ids اللي اختارها المستخدم عن طريق join
        const [voteItemRows] = await pool.query(`SELECT vi.id AS item_id
       FROM votes_items vi
       JOIN user_votes_items uvi ON uvi.item = vi.item
       WHERE uvi.user_vote_id = ? AND vi.vote_id = ?`, [userVoteId, voteId]);
        votedItemIds = voteItemRows.map((row) => row.item_id);
    }
